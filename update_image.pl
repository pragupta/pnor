...
while (@ARGV > 0){
    ...
    elsif (/^-vpd_file_dir/i)
    {
        $vpd_file_dir = $ARGV[1] or die "Bad command line arg given\n";
        shift;
    }
    else {
...
run_command("dd if=$vpd_file_dir/sysspd.dat of=$scratch_dir/hostboot.temp.bin ibs=256K conv=sync");
...
run_command("dd if=$vpd_file_dir/sysmvpd.dat of=$scratch_dir/hostboot.temp.bin  ibs=576K conv=sync");
...
run_command("dd if=$vpd_file_dir/sysmemvpd.dat of=$scratch_dir/hostboot.temp.bin ibs=192K conv=sync");
